...
**Will Google penalize the same text in different languages?**
No. A translation isn't a duplicate. A duplicate is identical content in one language across different URLs, solved with self-canonical. Translations are what hreflang is built for.

**Subdirectories, subdomains, or separate domains?**
For a business in Poland, subdirectories (`/ru/`, `/pl/`) on a single `.pl` domain. All SEO authority pools in one place and maintenance is simpler.

**Do I need hreflang with only two languages?**
Yes. Even for PL + EN. Without it Google may show the English version to a Polish user and vice versa, and both versions compete against each other in the results.

**Can I use machine translation?**
As a draft, yes. But publishing raw machine translation is a no: Google demotes it and the customer feels the fakeness. At minimum, native-speaker editing tuned to the market.
