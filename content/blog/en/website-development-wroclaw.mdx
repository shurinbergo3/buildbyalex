...
## Real-world case: a Ukrainian-cuisine chain in Wrocław

A small chain got in touch - three Ukrainian restaurants in Wrocław, one in Nadodrze and the others near the centre. They barely had a website, just Instagram and a listing on a food aggregator. People couldn't find them in Google, got the addresses and opening hours mixed up, and table bookings ran through DMs that nobody read in the evenings. The crowd was mixed: Ukrainians, Poles, and the international students from the university, from all over.

I built a fast site on Next.js: a page for each location with its address, menu, hours, and map, plus a shared section with the story and real photos of the food. Three languages from the start - PL, UA, EN - each on its own URLs with hreflang tags, so Google serves everyone the right version. Bookings went onto a form with a Telegram notification, and I set up Google Business for all three points with a single consistent address and phone.

Two months in: for searches like "українська кухня Wrocław" and "restauracja ukraińska Wrocław" they reached the local top, organic traffic to the site was up roughly threefold from almost nothing, and bookings through the site became a steady channel instead of a DM that got lost. The UA version pulled a real share of visitors - the regular local customer, not the one-night tourist. The whole build took three weeks.

That's exactly the point from above: a Wrocław service business doesn't need a platform, it needs a fast multilingual page people can find and book from on the spot.
