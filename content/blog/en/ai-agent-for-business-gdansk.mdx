...
## Real-world case: a freight forwarder at the Port of Gdańsk

A small forwarder near the port, five people, container runs from Gdańsk into Europe and back-haul out of Asia. Their problem was pure geography: about half the inquiries came in at night local time, right when the shipper in China or Turkey was mid-workday. Someone writes at 2 or 3 in the morning wanting a rough rate on a container and a transit time. The dispatcher saw it at 8, replied a few hours later, and by then the request had gone to whoever answered first. By their own count they were losing roughly 40% of the night flow - it just didn't survive until morning.

Here's what we did. I put an agent on the website and on WhatsApp, with an email connector so the overnight messages got picked up too. It answers 24/7 and asks what the dispatcher would ask anyway: cargo type, route and direction, volume, weight, the dates that matter, incoterms. From the loaded rate file it gives the client a ballpark range - with an honest note that a human confirms the final number - and drops the request into the CRM tagged "needs rate confirmation", context already filled in. In the morning the dispatcher isn't untangling a pile of emails, just working through clean cards.

The result over the first two months: night inquiries stopped falling through - the agent caught them on the spot and they reached the dispatcher alive. Total processed requests went up by about 35%, with nobody added to the team. The bigger win was first-response speed: in forwarding, whoever quotes first often gets the freight. The build took 4 weeks, and most of that went not into code but into getting the rate file into a shape the agent could read without slipping up.

That's what an agent means for a port business: not "a bot on the site" but a night shift covered in the time zones where your clients are wide awake.
