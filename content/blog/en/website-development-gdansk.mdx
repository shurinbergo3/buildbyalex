...
## Real-world case: a guesthouse by the sea in Sopot

A ten-room guesthouse a short walk from the Sopot pier came to me with a slow, heavy WordPress site. It took five or six seconds to load on a phone, the room gallery served photos at a couple of megabytes each, and it spoke exactly two languages - Polish, and clumsy English through an auto-translate plugin. In summer, by the owner's own account, half the visits bounced before they'd even seen the rooms. The Germans and Scandinavians who pour into Sopot hit the broken translation and left for Booking.com, where the owner was also handing over a commission on every night.

I built a new site on Next.js, statically rendered - pages pre-built, images in WebP and lazy-loaded. Mobile LCP dropped to under a second against the old five-plus. Four real language versions - PL, EN, DE, UA - each on its own URL with proper hreflang, not a translate button, and it wasn't just the marketing copy that got translated but the booking form and the confirmation emails too. I wired the booking so an inquiry lands in the owner's Telegram the moment it comes in; a guest in another time zone isn't going to wait for "good morning."

First season: conversion to an inquiry rose by about 45%, direct bookings climbed noticeably, and a chunk of traffic came off Booking.com, commission saved. The German traffic that used to bounce started reaching the booking step, so the DE version paid for itself. Speed did double duty: guests stopped leaving on load, and Google nudged the site higher in local results. The whole thing fit into four weeks.

The moral is simple. For a seaside business, speed and a genuinely multilingual site aren't about looking nice, they're rooms actually booked.
