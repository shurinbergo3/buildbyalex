...
/* "What Allegro really costs you" — two sliders, one uncomfortable yearly
   number, and how many own stores that money buys. The classic build-vs-rent
...
export function StoreAllegroCalc() {
  const t = useTranslations("services.store.calc");
  ...
  const monthly = sales * (fee / 100);
  const yearly = monthly * 12;
  const storesEquiv = Math.max(1, Math.floor(yearly / STORE_PRICE_ZL));
          ...
          <div className="relative mt-12 overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#0A0A0B] md:mt-14">
            ...
            <div className="relative grid gap-10 p-6 md:grid-cols-2 md:gap-12 md:p-10">
              ...
              <div className="flex flex-col rounded-[20px] border border-white/[0.09] bg-white/[0.035] p-6 md:p-8">
                ...
                <p
                  className="mt-3 text-[clamp(38px,4.6vw,54px)] font-semibold leading-none tracking-[-0.035em] tabular-nums"
                  style={{
                ...
                <p className="mt-1.5 text-[14px] text-white/50">{t("perYear")}</p>
                ...
                <div className="mt-6 grid grid-cols-2 gap-4 border-t border-white/[0.08] pt-6">
                  <div>
                    <p className="font-mono text-[20px] font-semibold tabular-nums text-white">
                      {nf.format(monthly)} zł
                    </p>
                    <p className="mt-1 text-[11.5px] uppercase tracking-[0.06em] text-white/40">{t("perMonth")}</p>
                  </div>
                  <div>
                    <p className="font-mono text-[20px] font-semibold tabular-nums text-white">
                      {storesEquiv}×
                    </p>
                    <p className="mt-1 text-[11.5px] uppercase tracking-[0.06em] text-white/40">{t("storesEquiv")}</p>
                  </div>
                </div>
